...
import com.gmail.tracebachi.DeltaBans.Velocity.Events.UnwarnEvent;
import com.gmail.tracebachi.DeltaBans.Velocity.Events.WarnEvent;
...
public class WarningListener implements Registerable
{
  ...
  public WarningListener(
    DeltaBansPlugin plugin, WarningStorage warningStorage, Map<Integer, List<String>> warningCommandsMap,
  ...
  private void onWarnChannelRequest(ReceivedMessage receivedMessage)
  {
    ...
    // fire event
    plugin.getServer().getEventManager().fire(new WarnEvent(warner, nameToWarn, message, isSilent));
  }
  ...
  private void onUnwarnChannelRequest(ReceivedMessage receivedMessage)
  {
    ...
    else
    {
      ...
      // fire event
      plugin.getServer().getEventManager().fire(new UnwarnEvent(warner, nameToUnwarn, amountToUnwarn, isSilent));
    }
  }
